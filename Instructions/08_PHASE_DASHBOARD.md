...
## Goal
Streamlit UI for monitoring and config. **Read-only for trading actions** — dashboard never places orders.

## Tasks

### 1. Implement `dashboard.py`

Required structure:
- Page config (wide layout)
- Top: 4 account metrics
- Sidebar: bot status + API call count + refresh button
- 5 tabs: Positions / Trades / Logs / Safety Events / Config

### 2. Critical rule
The dashboard **only reads from Alpaca and reads/writes to DB config**. It never calls `submit_order()`. Trading actions happen in `bot.py` only. The dashboard's only "write" actions are to the `bot_config` table.
...
# Expected: serves at http://localhost:8501, no crash

# 2. Open in browser → all 5 tabs render even with empty DB

# 3. Toggle kill switch in Config tab → reflected immediately
uv run python -c "import db; print(db.get_config('trading_enabled'))"
# Expected: matches dashboard toggle state

# 4. Save settings → new values appear in DB
# (toggle a number in dashboard, click save)
uv run python -c "import db; print(db.get_all_config())"
# Expected: new values visible

# 5. Dashboard renders without Alpaca connection
# Temporarily break ALPACA_API_KEY in .env, restart dashboard
# Expected: shows "Failed to load account" error but other tabs still render
```

If all 5 checks pass, Phase 8 is complete. Proceed to `09_PHASE_DEPLOY.md`.
